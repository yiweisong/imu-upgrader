...
class OpenIMU:
    ...
    def autobaud(self, ports):
        '''Autobauds unit - first check for stream_mode / continuous data,
            then check by polling unitc
            :returns:
                true when successful
        '''

        for port in ports:
            for baud in [460800, 230400, 115200, 57600, 38400]:
                self.open(port, baud)
    ...
    def sync(self, prev_byte=0, bytes_read=0):
        ...
        if S[0] == 85 and prev_byte == 85:  # VALID HEADER FOUND
            ...
            print(config_bytes, len(config_bytes), 'config_bytes')
            self.packet_type = '{0:1c}'.format(
    ...
    def parse_packet(self, payload, ws=False):
        ...
        if self.packet_type == 'S0':
            ...
            data = collections.OrderedDict(
                [('time', self.elapsed_time_sec), ('xAccel', accels[0]), ('yAccel', accels[1]), ('zAccel', accels[2]),
                 ('xRate', gyros[0]),
                 ('yRate', gyros[1]), ('zRate', gyros[2]), ('xMag', mags[0]), (
                     'yMag', mags[1]), ('zMag', mags[2]), ('xRateTemp', temps[0]),
                 ('yRateTemp', temps[1]), ('zRateTemp', temps[2]), ('boardTemp', temps[3]), ('GPSITOW', itow),
                 ('BITstatus', bit)])
            ...
            data = collections.OrderedDict(
                [('time', self.elapsed_time_sec), ('xAccel', accels[0]), ('yAccel', accels[1]), ('zAccel', accels[2]),
                 ...
                 ('yRateTemp', temps[1]), ('zRateTemp', temps[2]), ('boardTemp', temps[3]), ('counter', count),
                 ('BITstatus', bit)])
            ...
            data = collections.OrderedDict(
                [('time', self.elapsed_time_sec), ('rollAngle', angles[0]), ('pitchAngle', angles[1]),
                 ('yawAngleMag', angles[2]),
            ...
            data = collections.OrderedDict(
                [('rollAngle', angles[0]), ('pitchAngle', angles[1]), ('yawAngleMag', angles[2]),
            ...
            for i in range(2):
                tude_int32 = (16777216 * payload[2 * i + 18] + 65536 * payload[2 * i + 19] + 256 * payload[2 * i + 20] +
                              payload[21]) - 2147483648 if 16777216 * payload[2 * i + 18] + 65536 * \
                                                           payload[2 * i + 19] + 256 * payload[2 * i + 20] + payload[
                                                               21] > 2147483647 else 16777216 * payload[
                    2 * i + 18] + 65536 * payload[2 * i + 19] + 256 * payload[2 * i + 20] + payload[21]
            ...
            tude = [0 for x in range(3)]
            for i in range(2):
                tude_int32 = (16777216 * payload[2 * i + 24] + 65536 * payload[2 * i + 25] + 256 * payload[2 * i + 26] +
                              payload[27]) - 2147483648 if 16777216 * payload[2 * i + 24] + 65536 * \
                                                           payload[2 * i + 25] + 256 * payload[2 * i + 26] + payload[
                                                               27] > 2147483647 else 16777216 * payload[
                    2 * i + 24] + 65536 * payload[2 * i + 25] + 256 * payload[2 * i + 26] + payload[27]
                tude[i] = (360.0 * tude_int32) / math.pow(2, 32)
            ...
            data = collections.OrderedDict(
                [('bitStatus', bitStatus), ('hardware bit', hardwareBit), ('hardwarePowerBit', hardwarePowerBit),
            ...
            print(payload[0:4], payload[0], payload[1], payload[2], payload[3], 'payload')
